#!/bin/bash
set -e  # Abortar en caso de cualquier error
...
echo "slapd slapd/domain string doncom.com" | debconf-set-selections
echo "slapd shared/organization string DonComOrg" | debconf-set-selections
...
DEBIAN_FRONTEND=noninteractive apt-get install slapd ldap-utils -y
...
ldapsearch -x -H ldap://localhost -b dc=doncom,dc=com || true
...
print_hashes $GREEN "Instalando una implementación de código abierto de Java..."
...
print_hashes $MAGENTA "Instalando avahi-daemon y avahi-utils..."
apt-get install avahi-daemon avahi-utils -y

# Crear el directorio de servicios de Avahi si no existe
if [ ! -d "/etc/avahi/services" ]; then
    mkdir -p /etc/avahi/services
fi

# Crear el archivo de servicio LDAP para Avahi
print_hashes $CYAN "Creando el archivo de servicio LDAP para Avahi..."
tee /etc/avahi/services/ldap.service > /dev/null <<EOF
...
# Reiniciar el daemon de Avahi
print_hashes $RED "Reiniciando el daemon de Avahi..."
